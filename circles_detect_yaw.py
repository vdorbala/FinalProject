...
hardcoded_yaw = -90 #total yaw youd like to try
pauselength=1.5 #seconds to wait after first yaw to regroup and do second one
...
def find_circles(my_img):
    ...
    if pause active=False: #only if youre not currently in a pause
        if yawstep<yawsteps_number: #only if you havent done all of the yawsteps yet
            ...
            # SEND IT
            print('sending Yaw towards the window: ',hardcoded_yaw/yawsteps_number)
            vel_pub.publish(velocity)
